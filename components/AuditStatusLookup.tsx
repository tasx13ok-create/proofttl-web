...
type StatusResponse = {
  ...
  status?: string
  offer_type?: string
  scope?: { summary?: string; price_usd?: number; turnaround?: string; scoped_at_ms?: number } | null
  payment?: { state?: string; url?: string | null; paid_at_ms?: number | null }
...
const labels: Record<string, string> = {
  ...
  scoped: 'SCOPED — PAYMENT LINK NOT YET ATTACHED',
  payment_ready: 'SCOPE APPROVED — PAYMENT READY',
      ...
      {result?.ok && (
        <div style={{ marginTop: 22 }}>
          ...
          <div className="app-table">
            ...
            <div className="app-table-row"><span>Payment</span><span>{String(result.payment?.state || 'not_requested').replaceAll('_', ' ')}</span><span>{result.payment?.state === 'paid' ? '✓' : '—'}</span></div>
          </div>
          {result.payment?.state === 'ready' && result.payment.url && (
            <div className="hero-actions" style={{ marginTop: 18 }}>
              <a className="button button-primary" href={result.payment.url} rel="noreferrer">PAY APPROVED SCOPE →</a>
            </div>
          )}
          ...
          {result.status === 'paid' && <p className="app-note">Payment is recorded. Fulfillment is now the active step.</p>}
