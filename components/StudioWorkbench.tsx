...
function cloneDefaultWorkspace(): Workspace {
...
export default function StudioWorkbench() {
  ...
  function updateActiveFile(patch: Partial<StudioFile>) {
    ...
    setWorkspace((current) => ({
      ...current,
      files: current.files.map((file) => file.id === activeFile.id ? { ...file, ...patch } : file),
    }))
  }
  ...
  function addFile() {
    ...
    if (!name) return
    if (workspace.files.some((file) => file.name.toLowerCase() === name.toLowerCase())) return
  ...
  function deleteActiveFile() {
    ...
    setWorkspace((current) => {
      const files = current.files.filter((file) => file.id !== activeFile.id)
      return { ...current, files, activeFileId: files[0].id }
    })
  }
  ...
  function resetWorkspace() {
    ...
    setChat([])
    appendTerminal('output', 'Workspace reset to starter project.')
  }
  ...
  async function askModel(event: FormEvent) {
    ...
    try {
      const response = await fetch(`${API_URL}/studio/chat`, {
        ...
        credentials: 'include',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ message: modelMessage, language: activeFile.language, editor: activeFile.content, history }),
      })
  ...
  function rememberTerminal(raw: string) {
    setTerminalHistory((current) => {
      const next = [...current.filter((item) => item !== raw), raw].slice(-50)
      window.localStorage.setItem(TERMINAL_HISTORY_KEY, JSON.stringify(next))
      return next
    })
  ...
  function terminalKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    ...
    if (event.key === 'ArrowUp') {
      ...
      setTerminalValue(terminalHistory[nextIndex] || '')
      return
    }
  ...
  async function runTerminal(event: FormEvent) {
    ...
    if (command === 'help' || command === '?') return void appendTerminal('output', 'Commands: help, clear/cls, pwd, ls, cat <file>, echo <text>, status, models, files, save, reset, sandbox, open <target>. ↑/↓ recalls command history.')
    ...
    if (command === 'cat') {
      const file = workspace.files.find((item) => item.name.toLowerCase() === rest.toLowerCase())
      return void appendTerminal(file ? 'output' : 'error', file ? file.content : `File not found: ${rest}`)
    }
    ...
    if (command === 'save') {
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify(workspace))
      return void appendTerminal('output', `Saved ${workspace.files.length} files locally.`)
    }
    if (command === 'reset') return void resetWorkspace()
    if (command === 'sandbox') {
      setJobState('blocked')
      return void appendTerminal('output', 'Sandbox execution adapter is not connected yet. Real code execution will require an isolated per-job runtime with CPU/memory/time/network limits. Production shell access will remain forbidden.')
    }
    if (command === 'status') {
      try {
    ...
    if (command === 'open') {
      ...
      if (!target) return void appendTerminal('error', `Unknown open target: ${rest}`)
      appendTerminal('output', `Opening ${rest.toLowerCase()}…`)
      window.setTimeout(() => window.location.assign(target), 200)
      return
    }
  ...
  function insertAssistantCode(text: string) {
    ...
    if (!fenced?.[1] || !activeFile) return
    updateActiveFile({ content: fenced[1].trimEnd() + '\n' })
  }
    ...
    <div style={{ display: 'grid', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        <div>
          ...
          <h1 className="app-title" style={{ marginBottom: 6 }}>Code + models + terminal + workspace.</h1>
          <p className="app-copy" style={{ maxWidth: 900 }}>A browser-native development workspace for code, model-assisted editing, ProofTTL operations, and terminal-style workflows. Real execution is deliberately separated behind an isolated sandbox boundary.</p>
        </div>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span className="app-status">{runtime}</span>
          <span className="app-status">{workspace.files.length} FILES · {totalChars} CHARS</span>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '210px minmax(0,1.15fr) minmax(320px,.75fr)', gap: 14 }} className="studio-grid">
        <aside className="console-panel wide" style={{ minWidth: 0, alignSelf: 'stretch' }}>
          <div><h2 style={{ marginBottom: 2 }}>PROJECT</h2><small>{workspace.name}</small></div>
          <div style={{ display: 'grid', gap: 6, marginTop: 12 }}>
            {workspace.files.map((file) => (
              <button key={file.id} type="button" onClick={() => setWorkspace((current) => ({ ...current, activeFileId: file.id }))} style={{ textAlign: 'left', padding: '9px 10px', borderRadius: 8, border: file.id === workspace.activeFileId ? '1px solid rgba(103,232,249,.42)' : '1px solid rgba(148,163,184,.1)', background: file.id === workspace.activeFileId ? 'rgba(103,232,249,.08)' : 'rgba(255,255,255,.02)' }}>
                <strong style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis' }}>{file.name}</strong>
                <small>{file.language}</small>
              </button>
            ))}
          </div>
          <div style={{ display: 'grid', gap: 6, marginTop: 12 }}>
            <input value={newFileName} onChange={(event) => setNewFileName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') addFile() }} placeholder="new-file.py" aria-label="New Studio file name" />
            <button type="button" className="button" onClick={addFile}>+ FILE</button>
            <button type="button" className="text-link" onClick={renameActiveFile}>RENAME ACTIVE</button>
            <button type="button" className="text-link" onClick={deleteActiveFile} disabled={workspace.files.length <= 1}>DELETE ACTIVE</button>
          </div>
        </aside>

        <section className="console-panel wide" style={{ minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            <div><h2 style={{ marginBottom: 2 }}>{activeFile?.name || 'EDITOR'}</h2><small>{lineCount} lines · autosaved locally</small></div>
            <select value={activeFile?.language || 'text'} onChange={(event) => updateActiveFile({ language: event.target.value })} aria-label="Studio language">
              {LANGUAGE_OPTIONS.map((item) => <option key={item} value={item}>{item.toUpperCase()}</option>)}
            </select>
          </div>
        ...
        <section className="console-panel wide" style={{ minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 610 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 10 }}>
            <button type="button" className="text-link" onClick={() => quickAsk('Review the active file for bugs, security issues, and edge cases. Give concrete fixes.')}>REVIEW</button>
            <button type="button" className="text-link" onClick={() => quickAsk('Write tests for the active file. Return the best test file in a fenced code block.')}>TESTS</button>
            <button type="button" className="text-link" onClick={() => quickAsk('Refactor the active file for clarity and reliability without changing behavior. Return the full improved file.')}>REFACTOR</button>
            <button type="button" className="text-link" onClick={() => quickAsk('Explain the active file and how it fits into this project in plain English.')}>EXPLAIN</button>
          </div>
          <div style={{ flex: 1, overflow: 'auto', margin: '12px 0', display: 'grid', alignContent: 'start', gap: 10, maxHeight: 460 }}>
            {chat.length === 0 && <div className="app-empty"><strong>Ask about the active file or project.</strong>Studio sends the active file plus a compact project manifest to the routed coding model.</div>}
            {chat.map((item, index) => (
              <article key={`${item.role}-${index}`} style={{ padding: 12, borderRadius: 10, border: '1px solid rgba(148,163,184,.12)', background: item.role === 'user' ? 'rgba(103,232,249,.045)' : 'rgba(255,255,255,.025)', whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }}>
                ...
                {item.role === 'assistant' && /```/.test(item.content) && <button type="button" className="text-link" onClick={() => insertAssistantCode(item.content)} style={{ marginTop: 8 }}>REPLACE ACTIVE FILE WITH FIRST CODE BLOCK →</button>}
              </article>
            ))}
      ...
      <section className="console-panel wide">
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div><h2 style={{ marginBottom: 2 }}>TERMINAL</h2><small>POWERSHELL-STYLE · BROWSER-SAFE COMMAND MODE · HISTORY ENABLED</small></div>
          <span className="app-status">NO HOST SHELL</span>
        </div>
        <div style={{ marginTop: 12, minHeight: 230, maxHeight: 360, overflow: 'auto', padding: 14, borderRadius: 12, background: '#020608', border: '1px solid rgba(103,232,249,.13)', font: '12px/1.6 ui-monospace,SFMono-Regular,Menlo,Consolas,monospace' }}>
        ...
        <form onSubmit={runTerminal} style={{ display: 'flex', gap: 8, marginTop: 10 }}>
          ...
          <input value={terminalValue} onChange={(event) => setTerminalValue(event.target.value)} onKeyDown={terminalKeyDown} placeholder="help" aria-label="Studio terminal command" style={{ flex: 1 }} />
      ...
      <section className="console-panel wide">
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
          <div>
            <h2 style={{ marginBottom: 2 }}>EXECUTION JOBS</h2>
            <small>ISOLATED RUNNER BOUNDARY</small>
          </div>
          <span className="app-status">{jobState === 'blocked' ? 'SANDBOX NOT CONNECTED' : 'DESIGN READY'}</span>
        </div>
        <div className="app-empty" style={{ marginTop: 12 }}>
          <strong>Real code execution will never reuse the ProofTTL production process.</strong>
          The runner contract is intentionally separate: temporary job filesystem, language/runtime allowlist, CPU/memory/time limits, network policy, streamed stdout/stderr, package restrictions, and automatic destruction after completion.
          <button type="button" className="text-link" onClick={() => { setJobState('blocked'); appendTerminal('output', 'Execution requested, but no isolated sandbox provider is connected yet.') }}>CHECK RUNNER STATUS →</button>
        </div>
      </section>

      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
        <small>LOCAL WORKSPACE · cloud sync will require authenticated accounts</small>
        <button type="button" className="text-link" onClick={resetWorkspace}>RESET LOCAL WORKSPACE</button>
      </div>
    </div>
  )
}
