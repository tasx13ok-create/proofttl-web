...
export default function AuditPage() {
  ...
  return <main className="app-page audit-sales-page">
    ...
    <section className="onboarding-wrap"><div className="audit-sales-card">
      ...
      <section style={{ margin: '28px max(18px, min(34px, 4vw)) 0', minWidth: 0 }} aria-labelledby="audit-faq-heading"><div style={{ paddingBottom: 14 }}><p className="app-kicker">COMMON QUESTIONS</p><h2 id="audit-faq-heading" style={{ margin: 0, fontSize: 'clamp(24px,3vw,32px)' }}>Before you send the outputs.</h2></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,280px),1fr))', gap: 10, minWidth: 0 }}>{AUDIT_FAQ.map((item) => <article key={item.question} style={{ minWidth: 0, minHeight: 154, padding: 18, border: '1px solid rgba(148,163,184,.11)', borderRadius: 12 }}><h3 style={{ margin: '0 0 8px', fontSize: 16, overflowWrap: 'anywhere' }}>{item.question}</h3><p className="app-copy" style={{ margin: 0, fontSize: 12, lineHeight: 1.6, overflowWrap: 'anywhere' }}>{item.answer}</p></article>)}</div></section>
