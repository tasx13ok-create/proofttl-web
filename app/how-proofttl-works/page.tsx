...
export default function HowProofTTLWorksPage() {
  return <>
    ...
    >
      <section className="ptl-method-flow">{steps.map(([number,title,copy]) => <article key={number}><span>{number}</span><div><h2>{title}</h2><p>{copy}</p></div></article>)}</section>
      ...
      <section className="ptl-detail-cta"><div><span>One launch offer</span><h2>$1,500 Fact Audit</h2><p>10–25 real outputs or claims, deep verification of the highest-risk findings, human approval, proof/report delivery, and seven days of monitoring.</p></div></section>
    </ProductDetailShell>
    <style>{`
      .how-works-page { overflow-x: clip; }
      .how-works-shell { display:grid; grid-template-columns: minmax(0, 1fr); }
      .how-works-shell > * { grid-column: 1 !important; }
      .how-works-matrix .app-table-row { grid-template-columns: minmax(150px, .72fr) minmax(0, 1.7fr); }
      @media (max-width: 760px) {
        .how-works-shell { width: calc(100% - 24px); }
        .how-works-matrix .app-table-row { grid-template-columns: minmax(0, 1fr); }
        .how-works-page a { white-space: normal; overflow-wrap: anywhere; }
      }
    `}</style>
  </>
}
