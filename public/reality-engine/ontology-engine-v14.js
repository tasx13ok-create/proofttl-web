...
function resolveAction(defaultAction,override){
  if(override==='observe'||override==='control')return{kind:'observe',seedSalt:0,extraExperiment:false,courtOverride:'control'};
...
function epistemicCycle(opts={}){
  const state=sanitize(opts.barrierState),attack=Number(opts.attack)||0,pre=classify(state.events),defaultAction=actionFor(pre,attack,state),action=resolveAction(defaultAction,opts.barrierActionOverride),rootSeed=Number(opts.seed)||1,seed=action.seedSalt?mix(rootSeed^action.seedSalt):rootSeed;
  ...
  if(action.kind==='reroute-world-search'&&action.seedSalt)state.interventions.adversaryReroutes++;if(action.kind==='reroute-representation-search'&&action.seedSalt)state.interventions.languageReroutes++;
  const publicAttack=Number(result?.attack)||attack+1;let bonus=null;
  if(action.kind==='boost-intervention-search'&&action.extraExperiment&&result?.reason==='instrument_rejected'&&!result?.experimentAudit){bonus=mergeBonusExperiment(result,state,{...opts,seed},publicAttack);result={...result,experimentAudit:bonus.audit,experimentForge:bonus.experimentForge,experimentDepth:bonus.experimentForge.depth}}
  ...
  const post=classify(state.events);state.last={before:pre,action,defaultAction,after:post,attack:publicAttack,bonusExperiment:!!bonus};state.history.push(state.last);state.history=state.history.slice(-HISTORY_LIMIT);
  return{...result,barrier:{version:state.version,before:pre,after:post,action,defaultAction,bonusExperiment:!!bonus,interventions:{...state.interventions},immutableJudge:true,persistable:persistable(state)}};
}
...
add('V14 court control override',()=>{const a=resolveAction({kind:'reroute-world-search',seedSalt:123,extraExperiment:false},'control');if(a.kind!=='observe'||a.seedSalt!==0||a.extraExperiment)throw Error('control arm failed to suppress barrier intervention');return'method court can suppress a barrier intervention without changing the judge'});
...
return{...V13,VERSION,epistemicCycle,selfTest,__v14:{WINDOW,MIN_EVIDENCE,BONUS_EXPERIMENT_COOLDOWN,classify,initialState,sanitize,actionFor,resolveAction,persistable}};
});
