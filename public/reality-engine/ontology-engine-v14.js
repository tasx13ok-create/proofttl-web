...
function mergeBonusExperiment(result,state,opts,attack){
  const base=result?.experimentForge?.persistable||V13.__v13.initialState();
  const e=V13.__v13.sanitize(base);
  const audit=V13.__v13.audit(result,result?.ontology||opts.ontology||[],mix((Number(opts.seed)||1)^attack^0xB0A5E111));
  e.attempts++;
  if(audit?.certified){
    const signature=`${audit.candidate.featureId}|${audit.candidate.actionA}|${audit.candidate.actionB}`;
    if(!e.events.some(x=>x.signature===signature)){
      e.depth++;
      e.events.push({id:`E${e.depth}-${mix(signature.length^attack^0xB0).toString(16).toUpperCase().slice(0,5)}`,signature,attack,label:audit.candidate.label,featureId:audit.candidate.featureId,actionA:audit.candidate.actionA,actionB:audit.candidate.actionB,holdout:audit.holdout,transfer:audit.transfer,source:'barrier-instrument-boost'});
    }
  }
  e.history.push({attack,certified:!!audit?.certified,reason:audit?.reason||null,candidate:audit?.candidate||null,holdout:audit?.holdout||null,transfer:audit?.transfer||null,source:'barrier-instrument-boost'});
  e.history=e.history.slice(-72);
  state.lastBonusExperimentAttack=attack;
  state.interventions.instrumentBoosts++;
...
function epistemicCycle(opts={}){
  ...
  opts.progress&&opts.progress({phase:'barrier',generation:pre.n,formula:`BARRIER ${pre.kind.toUpperCase()} · ${action.kind}`});
  let result=V13.epistemicCycle({...opts,seed});
  if(pre.kind==='adversary'&&action.seedSalt)state.interventions.adversaryReroutes++;
  if(pre.kind==='language'&&action.seedSalt)state.interventions.languageReroutes++;
  const publicAttack=Number(result?.attack)||attack+1;
  let bonus=null;
  if(pre.kind==='instrument'&&action.extraExperiment&&result?.reason==='instrument_rejected'&&!result?.experimentAudit){
    bonus=mergeBonusExperiment(result,state,{...opts,seed},publicAttack);
    result={...result,experimentAudit:bonus.audit,experimentForge:bonus.experimentForge,experimentDepth:bonus.experimentForge.depth};
  }
  const event={attack:publicAttack,reason:result?.reason||'unknown',strictDepth:!!result?.strictDepth,languageEvent:!!result?.languageEvent,ok:!!result?.ok,experimentCertified:!!result?.experimentAudit?.certified};
  state.events.push(event);state.events=state.events.slice(-WINDOW);
  const post=classify(state.events);
  state.last={before:pre,action,after:post,attack:publicAttack,bonusExperiment:!!bonus};
  state.history.push(state.last);state.history=state.history.slice(-HISTORY_LIMIT);
...
function selfTest(){const tests=[...(V13.selfTest?V13.selfTest():[])],add=(n,fn)=>{try{tests.push({name:n,pass:true,detail:String(fn()||'pass')})}catch(e){tests.push({name:n,pass:false,detail:String(e.message||e)})}};
  ...
  add('V14 language diagnosis',()=>{const xs=[...Array.from({length:8},()=>({reason:'instrument_rejected'})),...Array.from({length:4},()=>({reason:'derived_sense_added'}))];if(classify(xs).kind!=='language')throw Error('language barrier missed');return'derived senses without K/Ω → language barrier'});
  ...
  add('V14 judge immutability',()=>{const s=initialState();for(const x of ['pThreshold','effectThreshold','nullCount','judge','certificate','depth'])if(Object.prototype.hasOwnProperty.call(s,x))throw Error(`barrier state owns ${x}`);return'barrier diagnoses process, never truth'});
  return tests;
}
