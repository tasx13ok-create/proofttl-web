...
function baseState(){return{version:'research-method-court-v1',scoreVersion:SCORE_VERSION,records:{},history:[],last:null}}
...
function utilityFromSummary(r){
  let u=0;
  if(r?.strictDepth)u+=12;
  if(r?.languageEvent)u+=9;
  if(r?.reason==='derived_sense_added'&&!r?.strictDepth&&!r?.languageEvent)u+=6;
  u+=4*Math.max(0,Number(r?.experimentDepth)||0);
  u+=5*Math.max(0,Number(r?.causalDepth)||0);
  u+=6*Math.max(0,Number(r?.conceptDepth)||0);
  u+=5*Math.max(0,Number(r?.sequentialDepth)||0);
  u+=6*Math.max(0,Number(r?.macroDepth)||0);
  return u;
}
function rescorePair(p){
  const treatment=clone(p?.treatment)||{},control=clone(p?.control)||{};
  const legacyT=Number(treatment.utility)||0,legacyC=Number(control.utility)||0;
  const baseDepths={experimentDepth:Math.min(Number(treatment.experimentDepth)||0,Number(control.experimentDepth)||0),causalDepth:Math.min(Number(treatment.causalDepth)||0,Number(control.causalDepth)||0),conceptDepth:Math.min(Number(treatment.conceptDepth)||0,Number(control.conceptDepth)||0),sequentialDepth:Math.min(Number(treatment.sequentialDepth)||0,Number(control.sequentialDepth)||0),macroDepth:Math.min(Number(treatment.macroDepth)||0,Number(control.macroDepth)||0)};
  const score=(r)=>{let u=0;if(r?.strictDepth)u+=12;if(r?.languageEvent)u+=9;if(r?.reason==='derived_sense_added'&&!r?.strictDepth&&!r?.languageEvent)u+=6;u+=4*Math.max(0,(Number(r?.experimentDepth)||0)-baseDepths.experimentDepth);u+=5*Math.max(0,(Number(r?.causalDepth)||0)-baseDepths.causalDepth);u+=6*Math.max(0,(Number(r?.conceptDepth)||0)-baseDepths.conceptDepth);u+=5*Math.max(0,(Number(r?.sequentialDepth)||0)-baseDepths.sequentialDepth);u+=6*Math.max(0,(Number(r?.macroDepth)||0)-baseDepths.macroDepth);return u};
  treatment.legacyUtility=legacyT;treatment.utility=score(treatment);control.legacyUtility=legacyC;control.utility=score(control);
  return{...p,scoreVersion:SCORE_VERSION,treatment,control,delta:treatment.utility-control.utility};
}
function sanitize(raw){if(!raw||raw.version!=='research-method-court-v1')return baseState();const records={};for(const [k,r] of Object.entries(raw.records||{})){const pairs=(Array.isArray(r.pairs)?r.pairs:[]).slice(-MAX_PAIRS).map(p=>Number(p?.scoreVersion)===SCORE_VERSION?p:rescorePair(p));records[k]={key:k,kind:String(r.kind||k.split(':')[0]||'unknown'),method:String(r.method||k.split(':')[1]||'unknown'),pairs,status:String(r.status||'calibrating'),lastPairAttack:Number(r.lastPairAttack)||0,lastDecisionAttack:Number(r.lastDecisionAttack)||0}}return{version:'research-method-court-v1',scoreVersion:SCORE_VERSION,records,history:Array.isArray(raw.history)?raw.history.slice(-HISTORY_LIMIT):[],last:raw.last||null}}
...
function persistable(state){const records={};for(const [k,r] of Object.entries(state.records))records[k]={...r,pairs:r.pairs.slice(-MAX_PAIRS)};return clone({version:state.version,scoreVersion:SCORE_VERSION,records,history:state.history.slice(-HISTORY_LIMIT),last:state.last})}
function courtSummary(state,currentKey,trial,selected){const rows=Object.values(state.records).map(r=>{const s=pairedStats(r);return{key:r.key,kind:r.kind,method:r.method,status:s.status,n:s.n,meanDelta:s.mean,ci95:[s.lower,s.upper],wins:s.wins,losses:s.losses,ties:s.ties,lastPairAttack:r.lastPairAttack}});return{version:state.version,scoreVersion:SCORE_VERSION,currentMethod:currentKey||null,trial:trial||null,selectedArm:selected||null,minimumPairs:MIN_PAIRS,decisionPairs:DECISION_PAIRS,maxPairs:MAX_PAIRS,practicalEdge:PRACTICAL_EDGE,probeCadence:DECIDED_PROBE_CADENCE,records:rows,immutableJudge:true,changesOmegaK:false,meaning:'barrier interventions must beat a matched no-intervention control on certified discovery utility; rejection alone is diagnostic, not scientific progress; unproven methods are suppressed and periodically re-probed',persistable:persistable(state)}}
...
add('V21 rejection is diagnostic not reward',()=>{const r={reason:'instrument_rejected',experimentForge:{depth:2},causalProgramVM:{depth:0},conceptForge:{depth:0},sequentialForge:{depth:0},interventionLanguage:{depth:0}};const u=utility(r,{experimentState:{depth:2}});if(u!==0)throw Error(`instrument rejection incorrectly earned utility ${u}`);return'plain instrument rejection earns zero method utility'});
add('V21 legacy pair rescoring',()=>{const p=rescorePair({attack:1,delta:1.5,treatment:{reason:'instrument_rejected',experimentDepth:2,utility:1.5},control:{reason:'attack_resisted',experimentDepth:2,utility:0}});if(p.delta!==0||p.treatment.utility!==0||p.scoreVersion!==SCORE_VERSION)throw Error('legacy pair was not migrated to discovery-only scoring');return'persisted V21 pairs migrate to the current utility definition'});
...
return{...V20,VERSION,epistemicCycle,selfTest,__v21:{MIN_PAIRS,DECISION_PAIRS,MAX_PAIRS,PRACTICAL_EDGE,DECIDED_PROBE_CADENCE,SCORE_VERSION,METHOD_KEYS,baseState,sanitize,pairedStats,utility,utilityFromSummary,rescorePair,methodFor,persistable}};
});
