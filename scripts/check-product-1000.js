...
const checks = [
  ...
  [layout.includes('<ProductNav />') && nav.includes('/workspace/') && nav.includes('/studio/') && nav.includes('/trust/') && nav.includes('PUBLIC_PRIMARY') && nav.includes('Start verification') && !nav.includes("{ href: '/worlds/', label: 'Worlds' }") && !nav.includes("{ href: '/cinematics/', label: 'Cinematics' }"), 'revenue-focused canonical product navigation'],
  [nav.includes('/work/') && nav.includes('/files/') && nav.includes('/automations/') && nav.includes('/money/') && nav.includes('/connections/'), 'global work/product areas preserved behind the app shell'],
  [nav.includes("{ href: '/foundry/', label: 'Foundry', ownerOnly: true }") && nav.includes('foundryAllowed') && nav.includes('/foundry/runs') && nav.includes('visibleAppPrimary'), 'Foundry navigation is authorized by the server instead of a public client allowlist'],
  ...
  [!home.includes('<nav className="nav shell"') && !home.includes('brand-mark">P') && home.includes('/proofttl-lockup.svg') && home.includes('CommercialHome'), 'homepage has one canonical nav, commercial rendering, and no legacy P branding'],
