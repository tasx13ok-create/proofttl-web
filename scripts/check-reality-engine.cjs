...
for(const required of ["OntologyEngineV21","schema:'reality-cloud-lab-v1'","methodCourtState","refusing silent reset","unreadable/corrupt","REALITY_LAB_CYCLES","REALITY_LAB_ALLOW_FRESH_BOOTSTRAP","missing; refusing unproven fresh bootstrap","bootstrapProvenance"])if(!labRunner.includes(required))throw new Error(`Reality Engine cloud lab runner contract missing: ${required}`)
...
if(!labWorkflow.includes('reality-engine-lab-certified-v2-'))throw new Error('Reality Engine cloud lab cache namespace must exclude pre-provenance lineage caches')
...
if(/REALITY_LAB_ALLOW_FRESH_BOOTSTRAP\s*:\s*['"]?1/.test(labWorkflow))throw new Error('Scheduled Reality Engine cloud lab must not silently authorize a fresh lineage bootstrap')
...
console.log('PASS Reality Engine: cloud lineage fail-closed — corrupt cached state cannot silently reset certified lab history')
console.log('PASS Reality Engine: cloud lineage provenance — a missing cache cannot masquerade as continuation of an existing certified lineage')
console.log('PASS Reality Engine: cloud cache provenance — pre-guard caches cannot be restored into the certified lineage namespace')
