...
const realityRedirect=(vercel.routes||[]).find(route=>String(route?.src||'').includes('reality-engine')&&Number(route?.status)>=300&&Number(route?.status)<400)
if(realityRedirect)throw new Error('Reality Engine public route is captured by a redirect')
...
console.log('PASS Reality Engine: production availability — browser runtime remains reachable while noindex boundary stays intact')
